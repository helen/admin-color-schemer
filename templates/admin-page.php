	...
	<form method="post" action="<?php echo admin_url( 'admin-post.php' ); ?>">
		...
		<table class="form-table">
			...
			$loops = array(
				...
				'button' => __( 'Button', 'admin-color-schemer' ),
			);
			foreach ( $loops as $handle => $nicename ): ?>

			<tr valign="top">
				<th scope="row"><label for="color-<?php echo $handle; ?>"><?php echo esc_html( $nicename ); ?></label></th>
				<td><input name="<?php echo $handle; ?>" type="text" id="color-<?php echo $handle; ?>" value="<?php echo esc_attr( $scheme->{$handle} ); ?>" class="regular-text" /></td>
			</tr>
